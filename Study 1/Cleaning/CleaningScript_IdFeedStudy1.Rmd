...
library(careless)
library(here)
here::i_am("Identity-Feedback.Rproj")
```
...
rawSec <- rawSec[rawSec$id!=60882,] # somehow 60882 exists for part 2 but not part 1, breaks script if I keep
...
for(n in 1:nrow(rawFirst)){
  ...
  for(c in 1:numIdentities){
    ...
    subMatW[c,as.numeric(unlist(selectWord))]  <- as.numeric(rawFirst[n,paste0("I",c,"_Sims_",as.numeric(unlist(selectWord) ))] ) / 11 # Weighted matrix
...
for(n in 1:nrow(rawSec)){
  ...
  numIdentities <- sum(!is.na(idMat[idMat$subID==i,2:9]))
    ...
    # identity strength
    I2Istrength <- strength(curGraphW)
    ...
    # each node's similarity to race: in terms of identities only
    ...
    # mean similarity from race (as feedback targeted at race): in terms of traits only
    traitCommRaceM <- mean(similarity(curGraph, method = "dice")[1,],na.rm=T)
    # each node's similarity to race (as feedback targeted at race): in terms of traits only
    traitCommRaceN <- similarity(curGraph, method = "dice")[1,]
    ...
    x1 <- cbind(subID=rep(i,numIdentities), id=idNames, idcode = 1:numIdentities, I2Ioutdeg = I2Ioutdeg[1:numIdentities], I2Iindeg = I2Iindeg[1:numIdentities], I2Ideg = I2Ideg[1:numIdentities],
                ...
                cI2T.Hub=cI2T.Hub[1:numIdentities], pI2A.Hub=pI2A.Hub[1:numIdentities],
                                   pI2T.Hub=pI2T.Hub[1:numIdentities], nI2A.Hub=nI2A.Hub[1:numIdentities], nI2T.Hub=nI2T.Hub[1:numIdentities], traitCommRaceN = traitCommRaceN[1:numIdentities])
  ...
  x3 <- cbind(subID = i, idNum = numIdentities, idEdgT = idEdgT, idDensity = density, idTrans = idTrans, idStrengHom = idStrengHom, idPosHom = idPosHom, idStigHom = idStigHom, idSizeSHom = idSzSHom, idSizeUHom = idSzUHom, idSizeDHom = idSzDHom, idInclusHom = idInclusHom, idDifferHom = idDifferHom, traitCommGlob = traitCommGlob,idCommGlob=idCommGlob,
              ...
              globEff = globEff, traitCommRaceM = traitCommRaceM)
...
tendSubDf <- as.data.frame(
  cbind(subID = uIds, 
        subTend = sumNomsAcrossIdentities
)
)
tendSubDf$subTend <- as.numeric(tendSubDf$subTend)
...
tendTraitDf$traitTend <- as.numeric(tendTraitDf$traitTend)
```
...
orderDf2 <- orderDf[order(orderDf$subID, orderDf$order),]
orderDf3 <- merge(orderDf2, qcidlong, by = c("subID","idcode","idq"))
orderDf4 <- merge(orderDf3, idLong, by = c("subID","idcode"))
orderDf5 <- merge(orderDf4, indDiff, by = "subID")
orderDf6 <- merge(orderDf5, globalIDDf, by = c("subID"), all.x = T, all.y=T)
orderDf <- orderDf6
...
for(i in uIds){
    ...
    raceConnect[1,] <- 1
    raceConnect[,1] <- 1
    raceConnectVect=raceConnect[lower.tri(raceConnect)==T]
    ...
    curSimDf <- cbind(subID = rep(i,length(simVect)), pair = 1:length(simVect), idSim = simVect, idtSim = simVectIT, idtpSim = simVectIT.p, idtnSim = simVectIT.n, posDist = posVect, strengDist = strengVect, stigDist = stigVect, sizeSDist = sizeSVector, sizeUDist = sizeUVector, inclusDist = inclusVector, differDist = differVector, raceConn = raceConnectVect)
...
```{r}
promptDf <- rawSec %>% select(Social_PromptA, Social_PromptL, Personal_Prompt, id, Condition, identity)
promptDf <- promptDf %>% mutate_at(c('Social_PromptA', 'Social_PromptL', 'Personal_Prompt'), ~na_if(., ''))
promptDf <- promptDf %>% mutate(Prompt = coalesce(Social_PromptA, Social_PromptL, Personal_Prompt)) %>% select(id, Prompt)
output <- vader::vader_df(promptDf$Prompt)
output <- output %>% select(compound, pos, neu, neg)
output <- output %>% rename(compound.v = compound, pos.v = pos, neg.v = neg, neu.v = neu)
promptDf <- cbind(promptDf, output)
promptDf <- promptDf %>% rename(subID = id)

indDiff <- merge(indDiff, promptDf, by = "subID", all.x = T)
fullSimDf <- merge(fullSimDf, promptDf, by = "subID", all.x=T)
veryLong <- merge(veryLong, promptDf, by = "subID", all.x=T)
orderDf <- merge(orderDf, promptDf, by = "subID", all.x = T)
```


```{r}
veryLong <- merge(veryLong, tendSubDf, by = "subID", all.x=T)
orderDf <- merge(orderDf, tendSubDf, by = "subID", all.x=T)
idShort <- merge(idShort, tendSubDf, by = "subID", all.x=T)
indDiff <- merge(indDiff, tendSubDf, by = "subID", all.x=T)

fullSimDf <- merge(fullSimDf, tendSubDf, by = "subID", all.x=T)
...
```{r}
# write.csv(demos, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/demos.csv")
# write.csv(veryLong, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/id2traitDf.csv")
# write.csv(orderDf, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/orderDf.csv")
# write.csv(idShort, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/id2traitShort.csv")
# write.csv(indDiff, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/indDiff.csv")
# write.csv(fullSimDf, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/identitySimDf.csv")
# write.csv(wideId, "~/Google Drive/Volumes/Research Project/Identity Feedback/Study 1/Data/Output/idWide.csv")
...
```{r}
arrow::write_parquet(demos, here("Study 1","Data","demos.parquet"))
arrow::write_parquet(veryLong, here("Study 1","Data","id2traitDf.parquet"))
arrow::write_parquet(orderDf, here("Study 1","Data","orderDf.parquet"))
arrow::write_parquet(idShort, here("Study 1","Data","id2traitShort.parquet"))
arrow::write_parquet(indDiff, here("Study 1","Data","indDiff.parquet"))
arrow::write_parquet(fullSimDf, here("Study 1","Data","identitySimDf.parquet"))
arrow::write_parquet(wideId, here("Study 1","Data","idWide.parquet"))
```
